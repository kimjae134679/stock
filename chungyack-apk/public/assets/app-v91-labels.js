...
function cyV91SetText(el,value){
  if(el&&el.textContent!==value)el.textContent=value;
}
function cyV91SetHtml(el,value){
...
function cyV91ApplyLabels(root=document){
  ...
  scope.querySelectorAll?.('[title],[aria-label]').forEach(el=>{
    for(const attr of ['title','aria-label']){
      ...
      if(!/추적/.test(current))continue;
      const next=cyV91Text(current);
      if(next!==current)el.setAttribute(attr,next);
    }
  });

  const nav=scope.querySelector?.('.nav-btn[data-page="tracking"]');
  cyV91SetHtml(nav,'<b>✓</b>신청');

  const trackingPage=scope.querySelector?.('.page[data-page="tracking"]');
  if(trackingPage){
    cyV91SetText(trackingPage.querySelector('.section-head h2'),'내 신청');
    cyV91SetText(trackingPage.querySelector('.section-head small'),'신청한 공고 · 결과 · 서류 · 계약');
  }

  const heroStats=[...(scope.querySelectorAll?.('.hero-stat span')||[])];
  heroStats.forEach(el=>{if(/내\s*(추적|신청)/.test(el.textContent||''))cyV91SetText(el,'내 신청')});
...
function cyV91Wrap(name,after){
  ...
  const wrapped=function(...args){
    ...
    try{after?.(...args)}catch(error){console.warn('[ChungYack] label refresh failed',error)}
    return result;
  };
...
if(typeof cyV90RenameStaticText==='function'&&!cyV90RenameStaticText.__cyV91Wrapped){
  const base=cyV90RenameStaticText;
  const wrapped=function(root=document){const out=base(root);cyV91ApplyLabels(root);return out};
  wrapped.__cyV91Wrapped=true;
  cyV90RenameStaticText=wrapped;
}
