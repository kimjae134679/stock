// v0.8.5 live: hidden notices stay as collapsed rows + one-tap tracking without editor popup.
...
// Hidden means "folded", not removed. Keep hidden items visible as compact placeholders.
cyV7Items=function(){
...
function cyV85CollapsedCard(item){
  ...
  return `<article data-cy-v82-card-id="${esc(item.id)}" class="cy-v85-collapsed-card ${saved?'is-saved':''}">
    ...
    <button type="button" class="cy-v85-restore" data-cy-v85-restore="${esc(item.id)}">복원</button>
  </article>`;
}
...
function cyV85WireRestoreButtons(){
  document.querySelectorAll('[data-cy-v85-restore]').forEach(b=>b.addEventListener('click',e=>{
    ...
    if(typeof cyToast==='function')cyToast('숨김을 해제했습니다.');
  }));
}

const _cyV85RenderRecommendations=renderRecommendations;
renderRecommendations=function(){
  _cyV85RenderRecommendations();
  cyV85WireRestoreButtons();
};
...
function cyV85HomeRow(x){
  ...
  return `<div class="cy-v85-home-collapsed" data-cy-v83-home-name="${esc(x.name||'')}">
    ...
    <button type="button" data-cy-v85-home-restore="${esc(x.name||'')}">복원</button>
  </div>`;
}

// Home renderer always keeps hidden rows in place, collapsed.
...
// One tap = tracked immediately. Editing remains available only from the Tracking tab's Edit button.
cyV4AddTracking=function(id){
  ...
  if(existingIndex>=0){
    ...
    if(existing.status==='취소/추적중단'){
      ...
      if(typeof cyToast==='function')cyToast('이미 추적 중인 공고입니다.');
    }
    return;
  }
  const f=typeof cyV84Finance==='function'?cyV84Finance(item):{summary:''};
  TRACKING.unshift({
    ...
    statusIcon:'✅',
    next:item.next||'',
    detail:[item.units||'',item.period||'',f.summary||''].filter(Boolean).join(' · '),
...
// Long-press wording: tracking no longer opens the edit dialog.
...
cyV83OpenActionSheet=function(ctx){
  ...
  if(track){
    const opp=ctx?.type==='home'?cyV82FindOpportunity(ctx.name):ctx?.item;
    track.textContent=opp&&cyV7IsTracked(opp)?'✓ 이미 추적중':'✓ 바로 추적하기';
  }
};
...
renderSettings=function(){
  _cyV85RenderSettings();
  const v=document.getElementById('settingsVersion');if(v)v.textContent=CY_V85_VERSION;
};
window.addEventListener('DOMContentLoaded',()=>{
  const v=document.getElementById('appVersion');if(v)v.textContent='v'+CY_V85_VERSION;
  const s=document.getElementById('settingsVersion');if(s)s.textContent=CY_V85_VERSION;
});
