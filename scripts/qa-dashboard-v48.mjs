...
async function run(name,viewport){
 ...
 const r=await page.evaluate(ids=>{const q=s=>document.querySelector(s),qa=s=>[...document.querySelectorAll(s)],missing=ids.filter(id=>!document.getElementById(id)),suspicious=qa('.mr-section').map(s=>{const b=s.getBoundingClientRect(),txt=(s.querySelector('.mr-body')?.textContent||'').trim();return{id:s.id,h:Math.round(b.height),text:txt.length}}).filter(x=>x.h>350&&x.text<30),shape=q('#cycle-visual .v48-shape'),dot=q('#cycle-visual .v48-current-dot'),lines=qa('#cycle-visual .v48-hist-line'),legend=qa('#cycle-visual .v48-legend-item'),raw=q('#cycle-visual .v48-actual'),scroll=q('#cycle-visual .v48-svg-scroll'),svg=q('#cycle-visual .v48-svg');return{missing,version:q('.top b')?.textContent||'',mark:q('.mr-buildmark')?.textContent||'',load:q('#loadWrap')?.getBoundingClientRect().height??-1,shapeText:shape?.textContent||'',lineCount:lines.length,legendCount:legend.length,currentCx:Number(dot?.getAttribute('cx')||0),lineLastXs:lines.map(x=>Number((x.getAttribute('points')||'').trim().split(/\s+/).at(-1)?.split(',')[0]||0)),rawOpen:raw?.open===true,rawText:raw?.textContent||'',shapeClient:Math.round(scroll?.clientWidth||0),shapeScroll:Math.round(scroll?.scrollWidth||0),svgWidth:Math.round(svg?.getBoundingClientRect().width||0),scrollWidth:document.documentElement.scrollWidth,innerWidth:window.innerWidth,suspicious};},ids);
 ...
 if(r.shapeScroll>r.shapeClient+3)throw new Error(name+': primary shape requires horizontal scroll '+r.shapeScroll+'/'+r.shapeClient);
