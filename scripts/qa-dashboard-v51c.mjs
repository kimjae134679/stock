...
async function run(name,viewport){
 const browser=await chromium.launch({headless:true});const page=await browser.newPage({viewport});const errors=[],logs=[];page.on('pageerror',e=>errors.push(String(e)));page.on('console',m=>logs.push(m.type()+': '+m.text()));
 await page.goto(base,{waitUntil:'domcontentloaded',timeout:60000});
 try{await page.waitForFunction(()=>document.querySelector('#cycle-visual .v51c-cycle'),null,{timeout:65000})}catch(e){const d=await page.evaluate(()=>({mark:document.querySelector('.mr-buildmark')?.textContent,classes:document.querySelector('#cycle-visual .mr-body')?.innerHTML?.slice(0,1200)||'',scripts:[...document.scripts].map(s=>s.src).filter(Boolean).slice(-8),load:document.querySelector('#loadWrap')?.style.cssText||''}));await page.screenshot({path:`qa-artifacts/${name}-timeout.png`,fullPage:true});console.error('DIAG',JSON.stringify(d),errors,logs.slice(-20));throw e}
