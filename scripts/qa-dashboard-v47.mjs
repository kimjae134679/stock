...
async function run(name,viewport){
 ...
 const qqq=page.locator('#cycle-visual button[data-ticker="QQQ"]').first();await qqq.click();await page.waitForFunction(()=>document.querySelector('#modal')?.classList.contains('open'));await page.waitForFunction(()=>document.querySelector('#modal .v46-cycle-svg'),{timeout:8000});await page.waitForTimeout(450);const modal=await page.evaluate(()=>({v47:document.querySelectorAll('#modal .v47-cycle-svg').length,dots:document.querySelectorAll('#modal .v47-same-dot').length,text:document.querySelector('#modalBody')?.textContent||''}));if(modal.v47<1||modal.dots<5||!modal.text.includes('과거 사이클 중간에 꽂아서 비교'))throw new Error(name+': modal v47 comparison missing '+JSON.stringify(modal));
