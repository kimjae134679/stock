...
function stats(segs,dir){const a=segs.filter(x=>x.direction===dir),d=a.map(x=>x.trading_days),m=a.map(x=>Math.abs(x.move_pct));return{count:a.length,mean_days:mean(d)?Math.round(mean(d)):null,median_days:median(d)?Math.round(median(d)):null,p25_days:percentile(d,.25)?Math.round(percentile(d,.25)):null,p75_days:percentile(d,.75)?Math.round(percentile(d,.75)):null,mean_move_pct:mean(m)!=null:+mean(m).toFixed(1):null,median_move_pct:median(m)!=null:+median(m).toFixed(1):null,p25_move_pct:percentile(m,.25)!=null:+percentile(m,.25).toFixed(1):null,p75_move_pct:percentile(m,.75)!=null:+percentile(m,.75).toFixed(1):null}}
