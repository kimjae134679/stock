...
const base=process.env.MR_QA_URL||'http://127.0.0.1:4173/reports/stable-v044.html?qa=1';
...
async function run(name,viewport){
  ...
  await page.waitForFunction(()=>document.querySelector('#cycle-visual .cycle-chart-svg'),{timeout:20000});
  await page.waitForTimeout(250);

  const result=await page.evaluate(ids=>{
    const q=s=>document.querySelector(s),qa=s=>[...document.querySelectorAll(s)];
    const missing=ids.filter(id=>!document.getElementById(id));
    const load=q('#loadWrap')?.getBoundingClientRect(),action=q('#action'),actionBody=q('#action .mr-body'),hero=q('#market'),cycle=q('#cycle-visual');
    ...
    return {
      missing,version:q('.top b')?.textContent||'',mark:q('.mr-buildmark')?.textContent||'',loadHeight:load?.height??-1,
      actionText:(actionBody?.textContent||'').trim(),actionHeight:Math.round(action?.getBoundingClientRect().height||0),actionFocus:!!q('#action .action-focus'),phaseGuide:!!q('#action .phase-guide'),
      heroSummary:!!q('#marketSummary'),heroAction:!!q('#marketAction'),heroText:hero?.textContent||'',heroHeight:Math.round(hero?.getBoundingClientRect().height||0),analogOpen:q('.analog-details')?.open??false,
      phaseSummaryParagraph:!!q('#segmentPhaseNow>p'),phaseActionCount:qa('#segmentPhaseNow .phase-action').length,pickTagCount:qa('#picks .pick-tags .info-tag').length,rawJsonMacro:(q('#macro .mr-body')?.textContent||'').includes('{"'),endmark:qa('.endmark').length,
      cycleChartCount:qa('#cycle-visual .cycle-chart-svg').length,cycleProxyCount:qa('#cycle-visual .cycle-proxy-card').length,cycleSummaryCount:qa('#cycle-visual .cycle-summary-grid span').length,cycleText:(cycle?.textContent||'').trim(),cycleHeight:Math.round(cycle?.getBoundingClientRect().height||0),
      foldCount:foldButtons.length,blankButtons:foldButtons.filter(b=>!(b.textContent||'').trim()).map(b=>b.dataset.fold),suspicious,foldedBad,scrollWidth:document.documentElement.scrollWidth,innerWidth:window.innerWidth
    };
  },ids);

  if(!result.version.includes('v0.4.4'))throw new Error(`${name}: version mismatch ${result.version}`);
  if(result.mark!=='MR044')throw new Error(`${name}: mark mismatch ${result.mark}`);
  ...
  if(result.cycleChartCount<1)throw new Error(`${name}: cycle trajectory chart missing`);
  if(result.cycleProxyCount<4)throw new Error(`${name}: cycle proxy cards missing ${result.cycleProxyCount}`);
  if(result.cycleSummaryCount<6)throw new Error(`${name}: cycle summary metrics missing ${result.cycleSummaryCount}`);
  if(!/거래일/.test(result.cycleText)||!/%/.test(result.cycleText))throw new Error(`${name}: cycle days/pct summary missing`);
  if(result.cycleHeight<300)throw new Error(`${name}: cycle section unexpectedly small ${result.cycleHeight}`);
  if(result.foldCount<17)throw new Error(`${name}: fold buttons ${result.foldCount}`);
  ...
  const qqq=page.locator('#cycle-visual [data-ticker="QQQ"]').first();
  if(!(await qqq.count()))throw new Error(`${name}: QQQ cycle proxy missing`);
  await qqq.click();
  await page.waitForFunction(()=>document.querySelector('#modal')?.classList.contains('open'));
  await page.waitForFunction(()=>document.querySelector('#modal .cycle-slot .cycle-chart-svg'));
  const modalCycle=await page.evaluate(()=>({charts:document.querySelectorAll('#modal .cycle-chart-svg').length,rows:document.querySelectorAll('#modal .cycle-compare-row').length,text:document.querySelector('#modal .cycle-slot')?.textContent||''}));
  if(modalCycle.charts<1||modalCycle.rows<4||!/과거/.test(modalCycle.text))throw new Error(`${name}: modal cycle visual incomplete ${JSON.stringify(modalCycle)}`);
  ...
  console.log(`[${name}] PASS`,result,folded,opened,modalCycle);
  await browser.close();
}

await run('desktop-v044',{width:1440,height:1000});
await run('mobile-v044',{width:390,height:844});
console.log('MR044 browser QA PASS');
