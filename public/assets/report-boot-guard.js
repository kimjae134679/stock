(function(){
  ...
  function stillLoading(){return /불러오는 중|불러오는중|동기화 중/.test(app.textContent||'') && app.children.length===0;}
  function fallback(d){
    const m=d&&d.market||{};
    ...
    <div class="notice boot-warning"><b>안전 모드로 표시 중.</b> 메인 렌더러가 늦거나 실패해도 핵심 데이터는 계속 표시됩니다. 페이지를 다시 열면 메인 UI를 재시도합니다.</div>`;
  ...
  setTimeout(async()=>{
    ...
    try{
      ...
      if(typeof window.render==='function'){
        try{window.render(d);return}catch(_){ }
      }
      fallback(d);
    }catch(err){
      if(stillLoading())app.innerHTML=`<div class="notice"><b>데이터 로드 실패</b><br>${esc(err&&err.message||err)}<br><small>네트워크가 복구되면 새로고침해 주세요.</small></div>`;
    }
  },1800);
})();
